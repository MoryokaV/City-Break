...
import { requiresAdminAuth, requiresAdminOrMasterAuth } from "../middleware/auth";
...
router.get("/fetchUsers", requiresAdminAuth, async (req: Request, res: Response) => {
  const users = await usersCollection.find({ city_id: req.session.city_id }).toArray();
...
router.post("/insertUser", requiresAdminAuth, async (req: Request, res: Response) => {
  ...
  user.admin = false;
  user.city_id = req.session.city_id;
